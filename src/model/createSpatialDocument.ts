...
import { anchorTransformFromBox, composeTransforms, transformFromBox } from './transform';

function nearestConcreteAncestor(namespace: string[], nodesByNamespace: Map<string, SpatialNode>): SpatialNode | undefined {
  for (let length = namespace.length - 1; length > 0; length -= 1) {
    const ancestor = nodesByNamespace.get(canonicalNamespacePath(namespace.slice(0, length)));
...
function flattenRenderable(nodes: SpatialNode[]): SpatialNode[] {
  return nodes.flatMap((node) => [node.renderable ? node : undefined, ...flattenRenderable(node.children ?? [])]).filter(Boolean) as SpatialNode[];
}

function assignUnionGroupsToTree(nodes: SpatialNode[], groupedNodes: SpatialNode[]): SpatialNode[] {
  const unionById = new Map(groupedNodes.map((node) => [node.id, node.unionGroupId]));

  return nodes.map((node) => ({
    ...node,
    unionGroupId: unionById.get(node.id) ?? node.unionGroupId,
    children: node.children ? assignUnionGroupsToTree(node.children, groupedNodes) : undefined,
  }));
}
...
export function createSpatialDocument(source: string): SpatialDocument {
  ...
  resolved.objects
    .sort((a, b) => a.namespace.length - b.namespace.length || a.lineNumber - b.lineNumber)
    .forEach((object) => {
      const parent = nearestConcreteAncestor(object.namespace, nodesByNamespace);
      ...
      const localTransform = object.renderable ? transformFromBox(object.box, object.transform) : anchorTransformFromBox(object.box, object.transform);
      const worldTransform = parent?.worldTransform ? composeTransforms(parent.worldTransform, localTransform) : localTransform;
