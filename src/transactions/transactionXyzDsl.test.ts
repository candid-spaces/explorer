...
import {
  ...
  transactionToXyzDslSceneInputSource,
...
describe('transactionsToXyzDslSource', () => {
  it('maps the current primary-key transaction into a scene-editor input declaration', () => {
    const cursor = transactionToXyzDslSceneInputSource(
      transaction('color: cyan', 0, '+4+1/+2+1/+0+1', 'originating-key'),
      'originating-key',
    );

    expect(cursor).toBe('"+4+1/+2+1/+0+1" : "color: cyan"');
    expect(transactionToXyzDslSceneInputSource(
